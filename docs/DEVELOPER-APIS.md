...
| `feed_item` | filter | `array{post, authorName, description, content, thumbnailUrl, thumbnailType, thumbnailLength} $item, Post $post` | `FeedService::buildItem()` — applied to every feed `<item>`, across the site-wide, per-category, per-tag, and per-author feeds. |
