...
| Name | Type | Args | Fires in |
|---|---|---|---|
| `comment_posted` | action | `Comment $comment` | [`SiteController`](../app/Controllers/SiteController.php)`::submitComment()` and `::submitPageComment()` — the two front-end comment-form handlers, after the comment is saved, before notification dispatch — and [`CommentReplyService`](../app/Services/CommentReplyService.php)`::reply()`, when a moderator replies from the admin Comments screen (the reply is already Approved and its `userId` is the moderator's; `$_POST` holds the admin's reply form, not a comment form, so don't rely on front-end form fields there). |
...
| `[lumora_folder_gallery]` | Core — [`FolderGalleryShortcode`](../app/Services/FolderGalleryShortcode.php), wired in `include/bootstrap.php` | `[lumora_folder_gallery folder_id="12" size="medium" link="full"]` — renders every image in Media folder `folder_id` as a row of thumbnails. `size` is `small` (default, ThumbnailService's cropped square "Thumbnail"), `medium`, `large`, or `full` — same size names the single Insert Image picker uses; a size larger than an image's own real dimensions never upscales it (ThumbnailService never generates a thumbnail bigger than its source), so that image renders at its own true, smaller size instead — `resolveImage()`'s `isCropped` flag (true only for a real generated crop, driving the `is-cropped` CSS modifier class) is what tells the theme's CSS when it's actually safe to force the fixed square box. `link` is `none` (default) or `full` (wraps each thumbnail in a link to the full-size image, joining the post's PhotoSwipe lightbox gallery the same way an Insert Image "Link To: Media File" image does). A missing/deleted folder, or a folder with no images, renders nothing. Inserted via the content editor's own dedicated "Insert Folder" toolbar button, not the generic shortcode picker — not registered with `register_shortcode()`. |
...
| `[lumora_link_directory]` | Link Directory plugin (LPP-026) — `LinkDirectoryShortcode`, wired in `link-directory.php` | `[lumora_link_directory]` with no attributes lists every category (and sub-category) with its own link count. `[lumora_link_directory category_id="3"]` (or `category="Fansites"`) shows that category's own links — title, thumbnail, description, and URL for each, from the plugin's own `link_directory_links`/`link_directory_categories` tables (unrelated to Media Folders or Downloads' own category taxonomy). `link_id`, the single-link variant, is unregistered — still typeable by hand. |
...
| `[lumora_gallery_album]`/`[lumora_gallery_newest]` | Lumora Gallery Shortcodes plugin (LPP-015) — `GalleryShortcode` (`content/plugins/lumora-gallery-shortcodes/src/`), wired in `lumora-gallery-shortcodes.php` | `[lumora_gallery_album album_id="12"]` (or `folder="..."`, or `count="N"` for the newest N, or `image_id="4,9,12"` for specific images — `album_id`/`folder` is optional with `image_id`, since an image id is already globally unique) and `[lumora_gallery_newest count="10"]` (newest N across every public album). Reads a *separately-configured Lumora Gallery site's own database* directly (`GallerySettingsService`, Settings screen credentials) — a different application's data, not anything in this site's own database. Since LPP-021, `GallerySettingsService` holds any number of named connections (keyed by slug) rather than one; both shortcodes accept a `gallery="{slug}"` attribute picking which connection to read from, defaulting to whichever connection `GallerySettingsService::defaultConnection()` names when omitted — every `album_id`/`image_id` in one shortcode instance resolves against that single connection. Partially registered (LPP-017): `[lumora_gallery_album]`'s picker form covers only the "whole album" variant (`album_id`, listed via `GalleryQueryService::listAlbums()` against the *default* connection only) plus the new `gallery` Select (choices = every configured connection) — `count`/`image_id`, and `album_id` from a non-default connection, stay typeable by hand. `[lumora_gallery_newest]`'s `count`/`gallery` are registered in full. An unconfigured or unreachable Gallery connection, or an unknown `gallery` slug, yields an empty `album_id` choice list / empty render rather than an error. |
...
| Name | Type | Args | Fires in |
|---|---|---|---|
| `plugin_action_links_{$slug}` | filter | `array<int, array{label: string, url: string}> $links` | `admin/views/plugins.php`, once per plugin (`$slug` is that plugin's own directory slug, e.g. `plugin_action_links_lumora-shield`) — return `[...$links, ['label' => 'Settings', 'url' => admin_url('...')]]` to add one-click links into the list-table row, grid card, and Details panel action areas, positioned before the built-in Details/Activate/Deactivate/Delete links. Register it from your plugin's own bootstrap file, not conditionally elsewhere — an inactive plugin's bootstrap never runs, so the filter simply never fires and nothing renders. See `content/plugins/lumora-gallery-shortcodes/lumora-gallery-shortcodes.php` for a plugin registering two links, or `content/plugins/lumora-shield/lumora-shield.php` for a plugin with no top-level menu of its own pointing at two different existing screens. |
...
| `admin_sidebar_alerts` | filter | `array<int, array{variant: string, url: string, title: string, label: string}> $alerts, User $currentUser` | `admin/views/layout-header.php`, on every admin screen (this file is the shared page chrome). Added for Visitor Stats' stale-GeoIP-data notice (LPP-025), generalizing the two condensed sidebar alerts core already showed for the `install/` directory and maintenance mode (LP-177) into a filter any plugin can add a third (or more) entry to, instead of those two staying a hardcoded special case. Each entry renders as `<a class="lp-alert lp-alert--{variant} lp-admin__sidebar-alert" href="{url}" title="{title}">{label}</a>` — `variant` is normally `'error'` or `'warning'` (any `.lp-alert--*` modifier works), `title` is the full explanation (the row itself truncates to one line), and `label` is trusted HTML, not escaped, same convention as `dashboard_widgets`' own output — inline tags like `<strong>`/`<code>` are fine, arbitrary markup from an untrusted source is not. See `content/plugins/visitor-stats/visitor-stats.php` for a real registration. |
...
Contact Forms (LPP-003), and Link Directory (LPP-026) instead each get a
real *top-level* menu entry of their own, since their admin screens are
their entire reason to exist — the same `{slug}Active` gate, just wrapping
a whole new top-level `$menu` array entry instead of a nested child. If
your plugin needs admin UI and
...
| `lp_emoji_picker_trigger_html` | filter | `string $html, array $args` (`label?`, `target?`) | Same plugin — backs the `lp_emoji_picker_button(array $args = [])` template tag, for rendering a self-contained trigger button outside the default editor toolbars. `comment_form()` (`include/comment-functions.php`) is this filter's first real caller, next to every comment/reply textarea; the dialog itself is `assets/js/emoji-picker.js`, a small standalone public script distinct from the admin editor's own bundle, loaded only when `lp_emoji_picker_enabled()` is true (see `FooterAssets::render()`). Returns `''` when the plugin is inactive/disabled, always safe to call unconditionally. |
