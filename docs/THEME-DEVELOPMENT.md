...
| `get_the_tags(Post\|int $post): array` | The Tags `$post` is assigned to (alphabetical). Empty array for a tagless post, never `null`. Accepts a bare post id as well as a `Post` object — useful on `search.php`, whose result cards are `SearchResult`s rather than `Post`s (only ever call it for a `'post'`-type result, since that's the only type with tags). |
| `post_has_tags(Post\|int $post): bool` | Whether `$post` has any tags at all. |
| `the_tags(Post\|int $post, string $before = '', string $sep = ', ', string $after = '')` | `get_the_tags()`, echoed as a `$sep`-joined list of links wrapped in `$before`/`$after`. Outputs nothing at all for a tagless post, so it's safe to call unconditionally. |
...
| Function | Purpose |
|---|---|
| `comment_form(Post\|Page $content, ?User $currentUser, array $guestFieldOptions = [], ?int $parentId = null, string $submitLabel = 'Post Comment'): void` | Renders one CSRF-protected form. Pass `$parentId` for a reply form. Fields added through the `comment_form_fields_after` filter (such as core's "Notify me of new comments" checkbox, `.lp-comment-form__subscribe`) appear just before the submit button with no theme changes needed. |
| `comment_list(array $tree, Post\|Page $content, ?User $currentUser, array $guestFieldOptions = [], int $depth = 0, int $maxDepth = 5, bool $avatarsEnabled = true, string $avatarRating = 'g', string $avatarDefault = 'mp', int $collapseThreshold = 10): void` | Renders the full nested comment thread, each reply's form embedded via `comment_form()`. A sub-thread with more than `$collapseThreshold` total replies renders inside a collapsed-by-default `<details class="lp-comment__thread">` instead of always-expanded. Each comment also gets, with no theme changes needed: a role badge in its meta line (`.lp-comment__role`, with `--post-author` or `--{role}` modifiers, and `.lp-comment--by-post-author` on the `<li>`), an "(edited)" marker (`.lp-comment__edited`), and an actions row (`.lp-comment__actions`) holding the reaction buttons (`.lp-comment-reactions`, `.lp-comment-reactions__button`/`__item`/`__emoji`/`__label`/`__count`, `.is-selected` for the visitor's own), a Quote button (`.lp-comment__quote-button`, revealed by script), and a Report control (`<details class="lp-comment-report">`, `.lp-comment-report__form`, `.lp-comment-report__thanks`). Reactions and reporting only appear when switched on in Settings › Discussion. Core loads the small script behind them through `footer_assets`. Plugins may add markup inside `.lp-comment__content` through the `comment_content_html` filter, so style that element's children generically. |
| `comments_template(array $vars = []): void` | Fires the `comments_template` action (passing `$vars` along), then renders the theme's own `comments.php` (no-op if it doesn't exist). Call this from `single.php`/`page.php` rather than writing comment markup by hand. When comment subscriptions are on, core uses that action to print the subscription panel (confirm/unsubscribe notices and the "Watch this thread" button) just before your `comments.php` — style it with `.lp-comment-subscription`, `.lp-comment-subscription__manage`, `.lp-comment-subscription__watch`, `.lp-comment-subscription__text`, and `.lp-comment-subscription__actions`; it reuses the shared `.lp-alert`/`.lp-button` classes for notices and buttons. A plugin can also replace the whole comment section through the `comments_template_html` filter (see `docs/DEVELOPER-APIS.md`), in which case neither the panel nor your `comments.php` renders. |
...
| `format_comment_content(string $raw): string` | Escapes raw comment text, auto-links bare URLs, wraps @mentions in `.lp-comment__mention`, and turns runs of lines starting with `>` into `<blockquote class="lp-comment__quote">` (text around a quote is then wrapped in `<p>`; a comment with no quote keeps plain `<br>` line breaks). |
| `comment_mentions(string $raw): array` | The lower-cased @handles in raw comment text. Handles are users' public author slugs, the same ones their author archive URL uses. |
