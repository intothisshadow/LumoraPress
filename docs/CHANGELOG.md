...
- Comment subscriptions (LP-012): a new "Let visitors subscribe to new comments on a post or page by email" option on Settings &rsaquo; Discussion (off by default) adds a "Notify me of new comments by email" checkbox to the comment form and a "Watch this thread" button for signed-in users. Guests confirm their subscription through an emailed link, which is only sent once their comment is approved, so a comment held as spam never causes mail to an address its author typed in. Every subscription email includes an unsubscribe link, and opening a confirm or unsubscribe link only shows a button on the post rather than acting straight away, so an email security scanner that opens links can't confirm or cancel anyone's subscription. Subscribers only hear about approved comments, and never about their own.
- In-app notifications (LP-012): a new Notifications screen in the admin, with an unread count in the sidebar, tells signed-in users about replies to their comments, new comments on their posts and pages, new comments on threads they watch, and (for users who can moderate) comments waiting for approval. Each notification links straight to the comment, notifications can be marked read individually or all at once, and the same screen lists every thread you're watching with a "Stop watching" button. Plugins can add their own notifications to this inbox too.
- A new `comment_form_fields_after` filter lets a plugin add fields to every comment and reply form, and the `comments_template` action now receives the comment section's view data (which post or page is being shown, and the current user).
- Comment reactions (LP-012): a new Reactions &amp; Reporting section on Settings &rsaquo; Discussion can add a single "Like" button or a set of emoji reactions (👍 ❤️ 😂 😮 😢) under every comment, with counts. Each person gets one reaction per comment: clicking it again removes it and picking another switches it, so reactions can't be stacked or repeated. An option limits reacting to signed-in users while everyone still sees the counts. Off by default.
- Reply to comments from the admin (LP-179): a new Reply action on each Approved or Pending comment in the admin Comments screen opens a reply form showing the original comment, and the reply is posted as you, threaded under it, without visiting the post. Replying to a Pending comment ("Approve and Reply") approves it first. A reply skips the holds a visitor's comment goes through and is approved outright, then notifies the same people a front-end reply would: subscribers, the person you replied to (in-app if they're a registered user, and by email when "Email a commenter when someone replies" is on), and anyone you @mention. You're never emailed about your own reply.
...
- A Comments API for plugin authors (LP-012), documented in a new "Comments API" section of `docs/DEVELOPER-APIS.md`: comment metadata (`CommentService::setMeta()`/`metaValue()`/`metaForComments()`, stored in a new table and removed with its comment), a `comment_moderation_status` filter for custom moderation rules, the previous status passed to `comment_status_changed`, a `notification_created` action for relaying notifications elsewhere, a `comment_content_html` filter for adding to each displayed comment, a `comment_reaction_types` filter for custom reactions, and a `comments_template_html` filter that lets a plugin replace the whole comment section with its own commenting system. Settings &rsaquo; Discussion now lists whichever emoji reactions are actually offered.
...
- The comment form's emoji picker now closes when you click outside it, as well as with Cancel or Escape.
- The admin Comments screen's search and filters (search, user, IP address, dates) now sit in a collapsible "Search &amp; Filter" panel like the Posts, Users, and Tags screens, collapsed by default and opened automatically while a filter is active, so the comment list is the first thing you see.
...
- The comment form's emoji picker opened with no emoji showing, so searching or browsing did nothing: its emoji list collapsed to zero height inside the dialog. It now shows and scrolls as intended.
- Permanently deleting a page now deletes its comments too, the same as deleting a post always has. Before, a deleted page's comments stayed in the database and could still appear in comment feeds, the admin Comments screen, and comment counts. Reactions, reports, and comment data added by plugins go with them, and so do email subscriptions to that page's comments. Moving a page to the Trash still keeps all of it, so restoring the page brings its comments back. Updating also removes comments that were left behind by pages deleted before this fix.
...
- The README's Cookies section described the session cookie as `PHPSESSID` and said it was never set for visitors who aren't signed in. It's actually named `lumora_press_session` and is set on every visit, because it also carries the security tokens that protect the comment form and other forms. The section now says so, for anyone basing a privacy or cookie policy on it.
...
- Security: the Plugins screen's "Cancel" action, for discarding a staged plugin ZIP mid-install, had no CSRF protection at all — the only action on that screen without one.
- The Plugins screen's row-view "Settings" chip (and any other plugin action link) was unreadable on hover once it had been visited at least once — a `:visited` link's own accent-blue text color out-specified the hover state's white text, leaving accent-blue text against the hover state's own solid accent-blue background (LP-171).
...
- Bulk activate/deactivate for plugins (LP-163): the Plugins screen's bulk-actions bar (previously "Delete" only) now also offers "Activate" and "Deactivate", applying to any selected mix of plugins in one request — the row checkbox column, previously shown only for inactive (deletable) plugins, now appears for every plugin so active ones can be selected too. Activate skips a slug that's unknown, already active, or requires a newer PHP version than the server has; Deactivate skips a slug that's unknown or already inactive; either way the results banner reports how many were skipped and why. Delete's confirmation prompt still fires exactly as before; Activate and Deactivate submit with no prompt, since both are freely reversible.
...
- Link an image to a URL or an existing page (LP-168): clicking an image in the WYSIWYG editor (Posts, Pages, Downloads) now shows a small floating "Insert/Edit Link" button right next to it, opening the same link dialog text already uses — type a URL, or pick an existing Post/Page from a live searchable list. The main toolbar's "Insert/Edit Link" button and the Cmd/Ctrl+K shortcut also now work correctly when an image is selected.
...
- Excerpts ran paragraphs together and leaked literal HTML entities (LP-161): `ContentRenderer::toPlainText()` (behind `get_the_excerpt()`, search snippets, and OG/meta description tags) `strip_tags()`-ed rendered HTML directly, which concatenates adjacent elements with no separator at all (`<p>A</p><p>B</p>` became `AB`, fusing the end of one paragraph into the start of the next) and never decodes entities, so a literal `&gt;`/`&amp;` surviving from the rendered HTML got double-escaped once re-escaped for display. A real word boundary now survives between elements, and entities are decoded, before the result is treated as plain text.
- Bare `/index.php` request 404'd instead of showing the homepage (LP-160): visiting the front controller file directly (bypassing the pretty-URL rewrite) matched no specific route and fell through to the hierarchical Page route, which looked for a Page literally named `index.php` and correctly reported it missing. `/index.php` now behaves exactly like `/`.
- HTML editor toolbar overflow (LP-159): the Post/Page Visual/HTML editor's toolbar collapsed most of its buttons — including Insert/Edit Link — behind a "..." overflow menu once the full button set no longer fit the editor's width. It now wraps onto a second row instead, every button visible at a glance.
- Path traversal in content image dimension resolution (LP-158): `ContentRenderer::resolveImageDimensions()` resolved an `<img>`/link URL's path onto disk without checking the result stayed inside the project's own tree, and `HtmlSanitizer::isSafeUrl()` allows any relative href through, `../` segments included — so a post/page body containing an href like `/../../config/config.php` (however it got there: hand-typed, imported, or pasted) could make the renderer read a file outside `content/uploads` for its pixel dimensions. The resolved path is now verified with `realpath()` against the project root before `getimagesize()` ever runs; a path that resolves outside it is treated exactly like a missing file, falling back to the `<img>`'s own width/height.
- Imported/absolute-URL content images never got lightboxed (LP-157): a post's content image was only made PhotoSwipe-lightbox-capable when its own or its linked `<a>`'s URL was relative — any absolute `https://...` URL (common in imported content, or a hand-typed link to a companion site's own media) was treated as "not an image link" outright and silently skipped, regardless of its file extension. Only the extension decides now. When the linked file lives on another host, its real dimensions can't be read from local disk — rather than guessing from the inline thumbnail (which visibly distorted the lightbox slide once the real image loaded in) or leaving the slide dimensionless (which PhotoSwipe turned out to need, without which it never advances past the inline thumbnail at all), the visitor's own browser resolves the real dimensions right as they click, before the lightbox opens.
...
- Contact Forms File Upload field and Reply-To (LPP-003): a form's field editor gained a File Upload type — a single attachment up to 5MB, restricted to a small set of common file types — which appears in the admin Submissions screen as a download link streamed through an authenticated route (never web-accessible directly). A new submission's notification email now sets its Reply-To to the submitter's own address, so replying from your inbox goes straight back to them.
- Collapsed admin sidebar with flyout submenus (LP-151): the admin sidebar now defaults to a classic-WordPress-style collapsed, icon-only width — hovering or keyboard-focusing a top-level item with children (Posts, Media Manager, Appearance, etc.) pops out its submenu next to it, instead of pushing everything below it down the page. A new toggle in the sidebar footer switches back to the previous full-width sidebar with its in-place expand/collapse, remembered per-browser. Unchanged on mobile, which keeps the existing off-canvas drawer regardless of this setting.
- Compressed database backups (LP-152): a database backup taken via an update or the on-demand "Back up now" button is now gzip-compressed (`db-{version}-{timestamp}.sql.gz`) on any server with the zlib extension — the overwhelming majority of hosts — shrinking its size considerably for a large site. A host without zlib falls back to the previous plain `.sql` dump automatically; both formats restore transparently, and an existing plain `.sql` backup taken before this change still restores normally. The Maintenance &rsaquo; Updates "Download database" link and manual-recovery instructions (see `docs/UPDATES.md`) now cover both extensions.
...
- Performance (LP-009): `PageService::listAllForTree()` now memoizes its result for the life of a request, eliminating a repeat identical query the admin "All Pages" tree view was making (once for its own rows, once again via the parent-select dropdown). Every write method that can change the tree evicts the cache, so a save still reads back fresh data in the same request.
- Page validation (LP-009): `PageService` now enforces the same title-required/title-length-cap validation and Html-content sanitization at save time that Posts already had — a gap where Pages accepted an empty or over-length title, or unsanitized stored Html content, that Posts had already closed.
- Pages admin permission testing (LP-009): the admin Pages screens' form-handling logic (quick edit, trash, restore, delete permanently, empty trash, duplicate, reposition, bulk actions, save, and revision restore) now lives in a dedicated `PagesController` class, the same extraction Posts went through (LP-082) — an internal refactor with no user-facing behavior change, except that the bulk-actions form and Save now show an error message instead of silently reloading the page if their session token expired.
- Audio/Video Player (LP-150): new "Insert Audio"/"Insert Video" toolbar buttons (both the Visual/HTML and Markdown editors) embed a playable file from the Media Library directly into post/page content via new `[lumora_audio]`/`[lumora_video]` shortcodes — previously, an uploaded audio or video file had no way to actually play in content, only a bare download link. The player is Plyr-enhanced for a consistent, accessible skin across browsers; a video's poster image and WebVTT caption/subtitle track (already settable on the Media item's own edit screen) now render automatically.
...
- Categories (LP-010): the admin Categories list's post-count column now links to the admin Posts list pre-filtered to that category, mirroring the same link Tags already has on its own post-count column. The list also gained a "Search & Filter" panel — search by name or slug, filter by parent category, minimum post count, or a created-date range — mirroring the admin Posts list's own Search & Filter panel. Categories can also now have their own image: a "Category Image" field on the Create/Edit Category form (choose from the Media Manager or upload directly), shown on that category's archive page on both bundled themes. A category's URL now reflects its parent/child position (e.g. `/category/tv-movies/star-trek`), mirroring how a Page's URL already does — an old bookmarked or indexed flat category link permanently redirects to the correct nested URL. The admin Categories list's unfiltered "All" view now renders a real depth-indented tree instead of a flat alphabetized table, with drag-and-drop (and keyboard-accessible Move Up/Move Down) reordering, mirroring the Pages tree view, plus per-row expand/collapse toggles for any category with children. A category's archive page can now also show a breadcrumb trail (e.g. "TV & Movies / Star Trek"), reusing the same breadcrumb styling a Page's own hierarchical URL already gets. The Create/Edit Category form also gained an "Archive display" field (Use theme default/Excerpt/Full Content), letting a single category's own archive page override the site-wide "Post display" Theme Option. The admin Categories list's bulk actions gained a "Change parent to…" option, reassigning every selected category's parent in one step. A category name now has to be non-empty and 191 characters or shorter, matching Posts/Pages' own title validation — previously an over-length name could reach the database with no warning. Also fixed: the Create/Edit Category form now correctly redisplays itself (with the error banner) when a save fails validation, instead of silently falling back to the Categories list.
- Performance (LP-010): `CategoryService` now memoizes category lookups, the category tree, and post counts for the life of a request, the same request-scoped caching LP-009 added to Pages. The admin Categories tree view no longer runs one post-count query per row, and every repeated category link on a page — breadcrumbs, a post listing's category badges, `sitemap.xml`'s full category loop — now resolves its ancestor chain once instead of once per occurrence. Every method that changes a category evicts the cache, so a save still reads back fresh data in the same request.
- Categories admin permission testing (LP-010): the admin Categories screen's form-handling logic (save, trash, restore, delete permanently, empty trash, reposition, bulk actions) now lives in a dedicated `CategoriesController` class, the same extraction Pages (LP-009) and Posts (LP-082) already went through — an internal refactor with no user-facing behavior change, except that Save and the bulk-actions form now show an error message instead of silently reloading the page if their session token expired.

### Fixed

- Tags (LP-011): the Create/Edit Tag form had the same form-redisplay bug just fixed on the Categories screen above — submitting a blank name fell through to the Tags list instead of redisplaying the form alongside the "A name is required." error banner. Fixed the same way: the validation-error path now sets the form's action/editing state explicitly before the GET-derived defaults apply.
...
- Visual/HTML post editor toolbar (LP-149): Insert Read More Tag moved earlier in the toolbar, right before Bullet List. Insert Shortcode also gained its own distinct icon — it was accidentally sharing Source Code's icon.
...
- Insert Shortcode toolbar button, in both the Markdown and WYSIWYG
  ...
  `[sdm_show_dl_from_category]`, the Downloads plugin's
  `[lumora_downloads]`, the Contact Forms plugin's `[contact_form]`
  (LPP-017), and the Lumora Gallery Shortcodes plugin's
  `[lumora_gallery_album]`/`[lumora_gallery_newest]` (LPP-017). Hidden
  entirely when nothing is registered (e.g. none of those plugins are
  active).
...
- Admin: internal ticket IDs and dev-doc/class-name references were
  leaking directly into user-facing copy an admin actually reads
  (LP-101) — Media Manager &rsaquo; Thumbnails' "Default featured image"
  field label, Settings &rsaquo; Media's "Show filenames in the
  lightbox" hint, Settings &rsaquo; Cache's "no reverse-proxy detected"
  warning banner (which literally said "see TODO.md's LP-037 entry"),
  and Settings &rsaquo; Security's Login Lockout hint ("see
  `LoginThrottle`") and User Enumeration list ("see `MEMORY.md`").
  Removed from all five; a full-codebase audit for both patterns found
