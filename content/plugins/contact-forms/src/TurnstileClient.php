...
/**
 * Same shape as ReCaptchaClient — off by default, fails closed when
 * enabled (see that class's docblock for why this differs from
 * AkismetClient's fail-open behavior).
 */
final class TurnstileClient
{
    ...
    {
        return ((string) $this->config->option('contact_forms_turnstile_enabled', '0')) === '1' && $this->secretKey() !== '';
    }

    public function siteKey(): string
    {
        return trim((string) $this->config->option('contact_forms_turnstile_site_key', ''));
    }
    ...
    {
        return trim((string) $this->config->option('contact_forms_turnstile_secret_key', ''));
    }
