...
/**
 * CRUD, hierarchy, and lifecycle for the Link Directory's own category taxonomy.
 *
 * @package LumoraPress
 * @subpackage Plugins
 * @author Ariane
...
/**
 * A Link has exactly one category (a single FK column, no join table), with
 * a trash-then-delete lifecycle and merge() so a leftover category is
 * removable even while links still reference it — mirrors the Downloads
 * plugin's own DownloadCategoryService, the closest existing precedent for
 * a plugin-owned, parent_id-nested taxonomy in this codebase.
 *
 ...
 */
final class LinkDirectoryCategoryService
{
    ...
    public function create(string $name, ?int $parentId = null): LinkDirectoryCategory
    {
        ...
        if ($category === null) {
            throw new RuntimeException('Failed to load the link directory category that was just created.');
        }

        return $category;
    }

    public function update(int $id, string $name, ?int $parentId = null): LinkDirectoryCategory
    {
        $existing = $this->findById($id);

        if ($existing === null) {
            throw new RuntimeException("Link directory category {$id} does not exist.");
        }
        ...
        if ($category === null) {
            throw new RuntimeException('Failed to load the link directory category that was just updated.');
        }
     ...
     * cascading the delete to them), clears category_id on any Link still
     * referencing this category (left Uncategorized, not deleted), then
    ...
    public function delete(int $id): bool
    {
        return (bool) $this->database->transaction(function () use ($id): int {
            ...
            $this->database->execute(
                'UPDATE ' . $this->linksTable() . ' SET category_id = NULL WHERE category_id = :category_id',
                ['category_id' => $id],
            );
     ...
     * than a raw bulk DELETE, so children and links are handled safely
     * instead of left with a dangling parent_id/category_id.
     *
    ...
    /**
     * Merges $sourceId into $targetId: links and child categories are
    ...
    public function merge(int $sourceId, int $targetId): bool
    {
        ...
        $this->database->transaction(function () use ($sourceId, $targetId): void {
            $this->database->execute(
                'UPDATE ' . $this->linksTable() . ' SET category_id = :target_id WHERE category_id = :source_id',
    ...
    public function findById(int $id): ?LinkDirectoryCategory
    {
    ...
    /**
     * @return array<int, array{category: LinkDirectoryCategory, linkCount: int}>
     */
    public function listAllWithLinkCounts(): array
    {
        $rows = $this->database->fetchAll(
            'SELECT c.*, COUNT(l.id) AS link_count
               FROM ' . $this->table() . ' c
               LEFT JOIN ' . $this->linksTable() . " l ON l.category_id = c.id AND l.trashed_at IS NULL
        ...
        return array_map(
            fn (array $row): array => ['category' => $this->hydrate($row), 'linkCount' => (int) $row['link_count']],
            $rows,
        );
    }
     ...
     * @return array<int, array{category: LinkDirectoryCategory, linkCount: int}>
     */
    public function listTrashedWithLinkCounts(): array
    {
        $rows = $this->database->fetchAll(
            'SELECT c.*, COUNT(l.id) AS link_count
               FROM ' . $this->table() . ' c
    ...
    /**
     * @return array<int, array{category: LinkDirectoryCategory, depth: int}>
     */
    ...
    /**
     * @param array<int, LinkDirectoryCategory> $categories
     * @return array<int, array{category: LinkDirectoryCategory, depth: int}>
     */
    ...
    public function linkCount(int $categoryId): int
    {
        return (int) $this->database->fetchColumn(
            'SELECT COUNT(*) FROM ' . $this->linksTable() . ' WHERE category_id = :category_id AND trashed_at IS NULL',
     ...
     */
    private function hydrate(array $row): LinkDirectoryCategory
    {
        return new LinkDirectoryCategory(
            id: (int) $row['id'],
    ...
    {
        return $this->tablePrefix . 'link_directory_categories';
    }
