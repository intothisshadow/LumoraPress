...
Embed albums and images from one or more separately-installed [Lumora Gallery](https://coding.unloved-heart.net/scripts/lumoragallery) sites into Lumora Press posts and pages.

This is an **optional integration plugin**. Lumora Press remains fully functional with it deactivated, and Lumora Gallery is never required or assumed installed. Lumora Gallery has no API of its own, so this plugin reads its database directly, read-only, via one or more separately-configured connections entered on its own Settings screen (Lumora Gallery &rsaquo; Settings). No changes to a Lumora Gallery installation are required for this half of the integration.
...
- `gallery="xena-archive"` on either shortcode picks which configured Gallery connection to read from, by its slug — only needed once more than one connection is configured; omitted, it uses whichever connection is marked Default. `album_id`/`image_id` in one shortcode instance always resolve against that same connection.
- `no_album_info="1"` on either shortcode drops the title and "View album"/"View gallery" link, leaving just the bare thumbnails — e.g. `[lumora_gallery_album album_id="12" no_album_info="1"]`.

Every thumbnail links to the real full-size image and opens it in Lumora Press's own existing PhotoSwipe lightbox — the same one every other gallery in Lumora Press uses, no extra JavaScript needed. Each rendered block also links back to the album below the images — or, for the multi-album and newest-across-the-gallery variants (no single album to link to), the Gallery site itself — unless `no_album_info="1"` is given, which omits that link entirely.
...
Lumora Gallery &rsaquo; Settings lists every configured Gallery connection, with Edit/Delete/"Make Default" actions and an "Add Connection" button — configure one per separately-installed Gallery site you want to embed from. Each connection needs:

- A label (shown in this list and in the Insert Shortcode picker's Gallery field) — its slug, used by a shortcode's own `gallery="..."` attribute, is derived from the label when the connection is first created and never changes afterward.
...
Left with no connection configured, both shortcodes simply render nothing — never a fatal error. A shortcode naming an unknown `gallery` slug (or a connection that gets deleted later) does the same.
